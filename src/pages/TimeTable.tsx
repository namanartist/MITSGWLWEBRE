...
import { Download, Search, FileText, ChevronRight, BookOpen, GraduationCap, Clock } from 'lucide-react';
...
export const TimeTable = () => {
  ...
  const timetableData = DEPARTMENTS.map(dept => ({
    deptName: dept.name,
    programs: [
  ...
  return (
    <div className="min-h-screen bg-[#fdfcf0] py-24 px-4 sm:px-6 lg:px-8">
      <SEO 
        title="Time Table" 
        description="Download the latest academic time tables for all undergraduate, postgraduate, and doctoral programs at MITS Gwalior."
      />
      <div className="max-w-7xl mx-auto">
        ...
        >
          <h1 className="font-serif text-5xl md:text-6xl mb-6">Academic Time Table</h1>
          <p className="text-xl text-black/60 max-w-3xl mx-auto">
            Access and download the detailed time tables for all courses and programs offered at MITS Gwalior.
          </p>
        </motion.div>
                    ...
                    <div className="space-y-2">
                      {prog.years.map((year, yIdx) => (
                        <a
                          key={yIdx}
                          href="#"
                          className="flex items-center justify-between p-4 bg-[#f8f9fa] rounded-xl hover:bg-green-50 transition-colors group"
                        >
                          <span className="text-sm font-medium">{year} Time Table</span>
                          <Download className="w-4 h-4 text-black/20 group-hover:text-green-500 transition-colors" />
                        </a>
                      ))}
                    </div>
                  </div>
